...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, className }) => {  
    const messageClasses = `flex h-fit min-w-0 items-center gap-4 px-8 py-3 text-white first:mt-auto ${
      message.role === "user" ? "bg-sce-grey-800 rounded-md" : ""
    } ${className || ""}`;
  
    const renderAvatar = () => (
      <div className="h-10 w-10 flex-shrink-0 self-start">
        {message.role === "assistant" ? (
          <img src="bot-avatar.png" className="border-sce-grey-800 rounded-md bg-white" alt="bot avatar" />
        ) : (
    ...
    return (
      <div className={messageClasses} key={message.id}>
        ...
        <div className="min-w-0 break-words">
          {message.id === "thinking" ? (
            <PulseLoader size={5} loading color="#ffffff" />
          ) : (
            <>
